...
while True:
    ...
    # pakai M_bev di sini
    frame_bird_eye = cv2.warpPerspective(frame, M_bev, (frame_width, frame_height))
    ...
    for contour in contours:
        ...
        if area > 300:
            mom = cv2.moments(contour)  # << ganti nama
            if mom["m00"] != 0:
    ...
    def hitung_posisi(contours_list):
        if len(contours_list) > 0:
            ...
            for cnt in contours_list:
                mom = cv2.moments(cnt)  # << juga ganti
                if mom["m00"] != 0:
    ...
    # ... (lanjutan sama seperti punyamu: pos_kiri, pos_tengah, offset, arah, imshow, dst.)
