...
def lane_offset(frame):
    ...
    mask = cv2.inRange(hsv, np.array([0, 0, 200]), np.array([180, 70, 255]))

    # ==== ROI MASKING (hanya area bawah tengah gambar) ====
...
with open(csv_path,"w",newline="") as f:
    ...
    while time.time() - start < durasi:
        ...
        bev = bird_eye(frm)
        cx, off, mask = lane_offset(bev)

        # Gambar area ROI di BEV
        h, w, _ = bev.shape
        cv2.rectangle(bev,
                      (int(w*0.1), int(h*0.5)),
                      (int(w*0.9), h),
        ...
        cv2.imshow("Mask Lane ROI", mask)  # opsional untuk debug
